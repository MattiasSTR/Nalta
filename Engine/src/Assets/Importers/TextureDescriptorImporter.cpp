...
namespace Nalta
{
    namespace
    {
        RHI::TextureFormat ParseCompression(const std::string& aStr, const bool aSRGB)
        {
            if (aStr == "BC1")   return aSRGB ? RHI::TextureFormat::BC1_SRGB  : RHI::TextureFormat::BC1_UNORM;
            if (aStr == "BC3")   return aSRGB ? RHI::TextureFormat::BC3_SRGB  : RHI::TextureFormat::BC3_UNORM;
            if (aStr == "BC4")   return RHI::TextureFormat::BC4_UNORM;
            if (aStr == "BC5")   return RHI::TextureFormat::BC5_UNORM;
            if (aStr == "BC7")   return aSRGB ? RHI::TextureFormat::BC7_SRGB  : RHI::TextureFormat::BC7_UNORM;
            if (aStr == "BC6H")  return RHI::TextureFormat::BC6H_UF16;
            if (aStr == "RGBA8") return aSRGB ? RHI::TextureFormat::RGBA8_SRGB : RHI::TextureFormat::RGBA8_UNORM;
            NL_WARN(GCoreLogger, "TextureDescriptorImporter: unknown compression '{}', defaulting to BC1", aStr);
            return RHI::TextureFormat::BC1_UNORM;
        }
    }
    ...
    std::unique_ptr<RawAssetData> TextureDescriptorImporter::Import(const AssetPath& aPath) const
    {
        ...
        const RHI::TextureFormat targetFormat{ ParseCompression(compression, sRGB) };
